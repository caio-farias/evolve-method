...
Vehicle.prototype.mitose = function () {
    var r = random(1);
    if (r < 0.001) {
...
Vehicle.prototype.mateSucess = function (mate, counter) {
    let choice = [];
    if (this.position == mate.position) {
        for (let i = 0; i < 4; i++) {
            choice.push(random_DNA(this, mate, i));
        }
    }
    this.n_mate++;
    counter++;
    return [new Vehicle(this.position.x, mate.position.y, choice), counter];
}
